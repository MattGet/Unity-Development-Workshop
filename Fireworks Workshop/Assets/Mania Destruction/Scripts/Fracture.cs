...
public class Fracture : MonoBehaviour
{
    ...
    private bool RequireExplodingFragments = false;
    private bool Exploded = false;
    private bool IncudeErase;
    private Rigidbody _rigidBody;
    private float _actualExplosionForce;
    ...
    void OnValidate()
    {
        if (this.transform.parent != null)
        {
            ...
            if (this.FractureBody == null && triggerOptions != null)
            {
                if (triggerOptions.triggerType != TriggerType.ExplosionEvent)
                {
                    ...
                    {
                        Debug.LogWarning($"Warning: Fracture Requires a Rigidbody");
                    }
                }
                else
                {
      ...
      bool AddErasable)
    {
        ...
        if (triggerOptions.triggerType == TriggerType.ExplosionEvent)
        {
            if (actualExplosionForce >= triggerOptions.minimumCollisionForce)
            {
                if (Exploded) return;
                Exploded = true;
                IncudeErase = AddErasable;
                _actualExplosionForce = actualExplosionForce;
                _forceMode = forceMode;
                _position = position;
                _range = range;
                _rigidBody = rigidBody;
                _upwardsmodifier = upwardsmodifier;
                //Debug.Log($"Erase = {IncudeErase}");
                this.ComputeFracture();
                RequireExplodingFragments = true;
                callbackOptions.onCompleted.AddListener(ExplodeCreatedFragments);
            }
        }
    }

    private void ExplodeCreatedFragments()
    {
        if (this.RequireExplodingFragments == true)
        {
            //Debug.Log("Exploding Fragments");
            RequireExplodingFragments = false;
    ...
    {
        List<Transform> fracs = new List<Transform>();
        foreach (Transform T in this.fragmentRoot.transform)
        {
            //Debug.Log($"Testing Fragment + {T.gameObject.name}");
            Rigidbody temp;
            if (T.gameObject.TryGetComponent(out temp))
            {
                fracs.Add(T);
                //Debug.Log($"Applying force of {(_actualExplosionForce / ((float)this.fractureOptions.fragmentCount / 10f))}");
                temp.AddExplosionForce((_actualExplosionForce / ((float)this.fractureOptions.fragmentCount / 10f)), _position, _range, _upwardsmodifier, _forceMode);

                //Debug.Log("Applied Explosion Force to Fragment");
            }
        }
        ...
        if (IncudeErase)
        {
            ...
            {
                yield return new WaitForSeconds(Time.deltaTime);
                //Debug.Log($"Bool Erase = {IncudeErase} and Name = {fracs[0].gameObject.name}");

                fracs[0].gameObject.AddComponent<ErasableBehavior>();
                //Debug.Log($"Added Erasable Component to {fracs[0].gameObject.name}");
                yield return new WaitForSeconds(Time.deltaTime);
                fracs[0].gameObject.transform.parent = _rigidBody.gameObject.transform.parent;
                //Debug.Log($"Set Fragment Parent to {fracs[0].gameObject.transform.parent.name}");
                fracs.RemoveAt(0);

            } while (fracs.Count > 0);
        }
            callbackOptions.onCompleted.RemoveAllListeners();
    ...
    private GameObject CreateFragmentTemplate()
    {
        ...
        var thisRigidBody = FractureBody;
